function wigb(a,scal,x,z,amx,indices)
...
%  Modified by Yunfeng Chen, Nov. 29, 2016 to plot the stacked trace
%  Y.C., Jan. 15, 2018, plot missing traces in red
%  IN indices: indices of empty traces
if_sum = false;
if if_sum
   a = [a, sum(a,2)/size(a,2)];
   x = [x x(end)+5*(x(end)-x(end-1))];
end
...
if (nargin <= 4); amx=mean(trmx); indices=[]; end;
